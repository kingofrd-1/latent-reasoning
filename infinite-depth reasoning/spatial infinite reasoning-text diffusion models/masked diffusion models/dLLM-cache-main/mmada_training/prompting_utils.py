...
class UniversalPrompting():
    ...
    def t2i_prompt(self, text_ids, image_ids, labels):
        ...
        for i in range(len(text_ids)):
            ...
            if self.max_text_len >= len(temp_ids): # padding outside task_id
                old_len = len(temp_ids)
                temp_ids = [self.pad_id] * (self.max_text_len - len(temp_ids)) + temp_ids # left padding
    ...
    def lm_prompt(self, text_ids, max_seq_len):
        ...
        for i in range(len(text_ids)):
            ...
            if max_seq_len >= len(temp_ids):
                temp_labels_ids = temp_ids + [self.text_tokenizer.eos_token_id] * (max_seq_len - len(temp_ids)) # right padding
    ...
    def mmu_prompt(self, image_ids, text_ids):
        ...
        for i in range(len(text_ids)):
            ...
            if max_text_len >= len(temp_ids):
                ...
                temp_ids = temp_ids + [self.text_tokenizer.eos_token_id] * (max_text_len - len(temp_ids)) # right padding
            ...
            for pos in range(len(temp_ids) - 1, -1, -1):
                if temp_ids[pos] == end_header_id: # the last <|end_header_id|>
                    end_header_pos = pos
                    break
            if end_header_pos != -1: # find the input length
    ...
    def its2ita_prompt(self, image_ids, text_ids, action_ids):
        """
        This is for image + text + state --> sub_goal(optional) + text + action
        """
        device = image_ids.device
        sequence_ids = []
        prompt_masks = []
        ...
        for i in range(len(text_ids)):
            ...
            #FIXME why there is a <|eot_id|> in front of <start_header_id>assistant<end_header_id>

            # temp_ids = text_ids[i] + [self.text_tokenizer.eos_token_id]
            temp_ids = text_ids[i]
            temp_ids += [self.sptids_dict['<|soa|>'].item()] + action_ids[i] + [self.sptids_dict['<|eoa|>'].item()]
            temp_ids += [self.text_tokenizer.eos_token_id]

            len_seq = len(temp_ids)

            # NOTE padding after acton tokens
            if max_text_len >= len_seq:
                # minus 1 because task token was prepended to the former image tokens
                temp_ids = temp_ids + [self.text_tokenizer.eos_token_id] * (max_text_len - len_seq) # right padding
                temp_masks = [1] * (len_seq + image_ids.shape[-1] + 3) + [0] * (max_text_len - len_seq)
            else:
                # should add the eos token
                temp_ids = temp_ids[:max_text_len - 1] + [self.text_tokenizer.eos_token_id]
                temp_masks = [1] * (len_seq + image_ids.shape[-1] + 3)  # +2 for two special tokens

            # prompting -- [task token] [soi] [image tokens] [eoi] [sot] [text tokens] [eot] [soa] [action tokens] [eoa]
            temp_label_ids = torch.cat([
                torch.tensor([self.ignore_id]).to(device),
                torch.tensor([self.ignore_id]).to(device),
                torch.ones_like(image_ids[i]) * self.ignore_id,
            ...
            return_temp_ids = torch.cat([
                self.sptids_dict['<|its2ita|>'].to(device),  # task token
                self.sptids_dict['<|soi|>'].to(device),
                image_ids[i],
                self.sptids_dict['<|eoi|>'].to(device),
                torch.tensor(temp_ids).to(device),
        ...
        return torch.cat(sequence_ids, dim=0), torch.cat(prompt_masks, dim=0), torch.cat(label_ids, dim=0), attention_masks
    ...
    def __call__(self, input, task, padding=True, config=None):
        ...
        if task == "t2i":
            ...
            image_ids, action_ids = input[0], input[2]
            text_ids = self.text_tokenizer(input[1])['input_ids']
            sequence_ids_with_masks = self.its2ita_prompt(image_ids, text_ids, action_ids)
