...
"""Jax implementation of LLAMA2-like Transformer with model sharding.
  ...
  def setup(self):
    ...
    self.wq = nn.Dense(
        ...
        kernel_init=nn.with_logical_partitioning(
            nn.linear.default_kernel_init, ('embed', 'qkv')
        ),
    )  # fsdp, tensor
    self.wk = nn.Dense(
    ...
    self.wo = nn.Dense(
        ...
        use_bias=False,
        kernel_init=nn.with_logical_partitioning(
            nn.linear.default_kernel_init, ('qkv', 'embed')
        )
    )
    if self.use_attn_dropout and self.dropout_rate > 0.0:
...
class FeedForward(nn.Module):
  ...
  def setup(self):
    ...
    self.w1 = nn.Dense(
        hidden_dim,
        use_bias=False,
        kernel_init=nn.with_logical_partitioning(w_init, ('embed', 'mlp')),
    )
    self.w2 = nn.Dense(
        self.dim,
        use_bias=False,
        kernel_init=nn.with_logical_partitioning(w_init, ('mlp', 'embed')),
    )
    self.w3 = nn.Dense(
        hidden_dim,
        use_bias=False,
...
class TransformerBlock(nn.Module):
  ...
  def setup(self):
    ...
    self.attention = Attention(
        ...
        causal=args.causal,
        use_attn_dropout=args.use_attn_dropout,
    )
  ...
  def __call__(self, x, cond=None, train=False, output_channels=None):
    ...
    if cond is not None:
      ...
      logits = nn.Dense(
          ...
          use_bias=False,
          kernel_init=nn.with_logical_partitioning(
              nn.initializers.zeros, ('embed', 'output_vocab')
          ),
