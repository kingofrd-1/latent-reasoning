...
r"""A config for training MD4 on downsampled ImageNet 64x64."""
...
def get_config() -> config_dict.ConfigDict:
  ...
  config.dataset = "downsampled_imagenet/64x64"
  config.classes = -1

  config.task_type = "image"  # text or image
  config.model_type = "md4"
  config.data_shape = (64, 64, 3)
...
def sweep(add: abc.Callable[..., None]):
  ...
  # Default: linear schedule, cosine sampling grid
  add(
      dataset="downsampled_imagenet/64x64",
      sampling_grid="cosine",
      noise_schedule="linear",
      n_layers=8,  # use 24 for best likelihood in the paper
      n_dit_layers=20,  # use 12 for best likelihood in the paper
  )
  # # Cosine schedule: worse likelihood, for best sample quality
  # add(
  #     dataset="downsampled_imagenet/64x64",
