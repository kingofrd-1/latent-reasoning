...
from generation_utils.dream_generation_utils_pd import DreamGenerationMixin, DreamGenerationConfig
...
class DreamRotaryEmbedding(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x, position_ids, prv_cache_position=None):
        ...
        #print("Rotary before cache:", cos.shape, sin.shape)

        if prv_cache_position is not None:
            #print(prv_cache_position[:, :, None].shape, cos.shape, cos[prv_cache_position[:, :, None].expand_as(cos)].shape)
            q_cos = cos[~prv_cache_position[:, :, None].expand_as(cos)].view(x.shape[0], -1, cos.shape[2])
            q_sin = sin[~prv_cache_position[:, :, None].expand_as(sin)].view(x.shape[0], -1, cos.shape[2])

            k_cos = cos[prv_cache_position[:, :, None].expand_as(cos)].view(x.shape[0], -1, cos.shape[2])
            k_sin = sin[prv_cache_position[:, :, None].expand_as(sin)].view(x.shape[0], -1, cos.shape[2])
            k_cos = torch.cat([k_cos, q_cos], dim=1)
            k_sin = torch.cat([k_sin, q_sin], dim=1)
            # after cache:", q_cos.shape, q_sin.shape)
            #print("K Rotary after cache:", k_cos.shape, k_sin.shape)
            return (q_cos.to(dtype=x.dtype), q_sin.to(dtype=x.dtype)), (k_cos.to(dtype=x.dtype), k_sin.to(dtype=x.dtype))
        else:
            #print(cos.shape, sin.shape)
            return (cos.to(dtype=x.dtype), sin.to(dtype=x.dtype)), (cos.to(dtype=x.dtype), sin.to(dtype=x.dtype))
...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        use_cache: bool = False,
        cache_position: Optional[torch.LongTensor] = None,
        prv_cache_position: Optional[torch.LongTensor] = None,
        prefill_cache_position: Optional[torch.LongTensor] = None,
        position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # will become mandatory in v4.46
        k_position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        if output_attentions:
            ...
            if use_cache:
                raise NotImplementedError(
                    "Cache is not supported in DiM15Attention with output_attentions = True. Please use `use_cache=False` to avoid errors."
                )
        ...
        if use_cache and not past_key_value.is_empty():
            ...
            if attention_mask != 'full':
                if prv_cache_position is not None:
                    attention_mask = attention_mask[:, :, (prv_cache_position).sum(dim=-1)[0]:, :]
                else:
                    attention_mask = attention_mask[:, :, (prefill_cache_position).sum(dim=-1)[0]:, :]
        else:
            k_len = q_len
        ...
        if use_cache and not past_key_value.is_empty() and prv_cache_position is None: 
            # deal with the cache when prefill is not refresh but decode is refresh
            cos = cos[:, prefill_cache_position.sum(dim=-1)[0]:]
            sin = sin[:, prefill_cache_position.sum(dim=-1)[0]:]

        if past_key_value is not None and cache_position is not None:
            cache_kwargs = {
                "cache_position": cache_position,
                "prv_cache_position": prv_cache_position,
                "prefill_cache_position": prefill_cache_position,
            }  # Specific to RoPE models
            past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)

        query_states = apply_rotary_pos_emb(query_states, cos, sin)

        k_cos, k_sin = k_position_embeddings
        ...
        #if self.layer_idx == 0:
        #    print(attn_output[:, 0], attn_output.shape)    
        #    torch.save({
        #        'attn': attn_output,
        #        'query': query_states,
        #        'key': key_states,
...
class DreamDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        prefill_cache_position: Optional[torch.LongTensor] = None,
        position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # will become mandatory in v4.46
        k_position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # will become mandatory in v4.46
        **kwargs,
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def forward(
        ...
        if use_cache and past_key_values is None:
            raise ValueError("`use_cache=True` but `past_key_values=None`. Please set `past_key_values` to a valid value.")

        if position_ids is None:
            logger.warning_once(
                "Please check if the position_ids are being passed correctly. Currently is None"
            )

        hidden_states = inputs_embeds

        # create position embeddings to be shared across the decoder layers
        position_embeddings, k_position_embeddings = self.rotary_emb(hidden_states, position_ids, prv_cache_position)
