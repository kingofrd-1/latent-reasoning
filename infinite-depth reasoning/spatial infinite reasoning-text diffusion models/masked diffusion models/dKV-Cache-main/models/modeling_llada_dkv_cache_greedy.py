...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        if self.config.rope:
            ...
            q, k = self.rotary_emb(q, k, transfer_idx is not None and self._kv_cache)
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if layer_past is not None and use_cache:
            ...
            cur_transfer_idx = (~transfer_idx[1]).unsqueeze(-1).expand(B, -1, D)

            cur_past_k = past_k[cur_transfer_idx].view(B, -1, D)
            cur_past_v = past_v[cur_transfer_idx].view(B, -1, D)
            cur_past_k = cur_past_k.contiguous()
            cur_past_v = cur_past_v.contiguous()
            ...
            if self._kv_cache:
                ...
                k = torch.cat([k_proj, cur_past_k], dim=1)
            
                v_proj = self.v_proj(x_normed)
                v = torch.cat([v_proj, cur_past_v], dim=1)

                new_decode_idx = torch.nonzero(transfer_idx[1], as_tuple=True)[1].view(B, -1).unsqueeze(-1).expand(B, -1, D)
                past_k.scatter_(1, new_decode_idx, k_proj)
                past_v.scatter_(1, new_decode_idx, v_proj)
            else:
