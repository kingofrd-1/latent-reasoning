...
from generation_utils.dream_generation_utils import DreamGenerationMixin, DreamGenerationConfig
...
class DreamRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids, prv_cache_position=None):
        ...
        if prv_cache_position is not None:
            ...
            k_cos = torch.cat([q_cos, k_cos], dim=1)
            k_sin = torch.cat([q_sin, k_sin], dim=1)
...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        #    attention_scores = torch.einsum('bhqd,bhkd->bhqk', query_states, key_states)
        #    attn_weights = torch.nn.functional.softmax(attention_scores / math.sqrt(query_states.size(-1)), dim=-1)
        #    import time
        #    torch.save({
        #        'attn': attn_weights, 
        #    }, f'notebook/dream_values_layer_10/attention_weight_{time.time()}_layer{self.layer_idx}.pt')
