...
from .cache_utils import PrefillCache, DynamicCache, PrefillDynamicCache
        ...
        **model_kwargs,
        ...
        if generation_config.use_cache:
            if generation_config.cache_type == "prefill":
                ...
                past_key_values = PrefillDynamicCache(self.config.num_hidden_layers)
                cache_position = ~(x == mask_token_id)
                cache_position = torch.cat([cache_position[:, 1:], cache_position[:, -1:]], dim=-1)
                prv_cache_position = None
                prefill_cache_position = None
            elif generation_config.cache_type == 'pd':
                past_key_values = PrefillDynamicCache(self.config.num_hidden_layers)
                prefill_cache_position = ~(x == mask_token_id)
                prefill_cache_position = torch.cat([prefill_cache_position[:, 1:], prefill_cache_position[:, -1:]], dim=-1)
        ...
        for i in range(steps):
            ...
            if not generation_config.use_cache:
                prefill_cache_position, cache_position, prv_cache_position = None, None, None
            else:
                if generation_config.cache_type == "prefill":
                    prefill_cache_position = None
                    pass
                    ...
                    if i % generation_config.cache_steps == 0:
                        prv_cache_position = None
                        past_key_values.refresh_decode_cache()
                elif generation_config.cache_type == "pd":
                    if i % generation_config.cache_steps == 0:
                        prv_cache_position = None
                        past_key_values.refresh_decode_cache()
            ...
            elif generation_config.use_cache and generation_config.cache_type == "pd" and i != 0:
                step_x = x[~prefill_cache_position].view(x.shape[0], -1)
            ...
            if logits.shape[1] < L:
                ...
                if prv_cache_position is None:
                    prv_cache_position = prefill_cache_position
            ...
            if generation_config.use_cache:
                ...
                if generation_config.cache_type == "prefill":
                    ...
                    if prv_cache_position is None:
                        ...
                        cache_position = None  
                elif generation_config.cache_type == "decoded" or generation_config.cache_type == "pd":
                    prv_cache_position = cache_position
