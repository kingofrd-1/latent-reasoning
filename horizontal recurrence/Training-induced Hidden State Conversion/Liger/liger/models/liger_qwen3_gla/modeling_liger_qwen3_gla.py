...
from .configuration_liger_qwen3_gla import LigerQwen3GLAConfig

logger = logging.get_logger(__name__)

class LigerQwen3GatedLinearAttention(nn.Module):
    def __init__(
        self, 
        config: LigerQwen3GLAConfig,
...
class LigerQwen3DecoderLayer(Qwen3DecoderLayer):
    def __init__(self, config: LigerQwen3GLAConfig, layer_idx: int):
        ...
        self.self_attn = LigerQwen3GatedLinearAttention(config, layer_idx)

class LigerQwen3PreTrainedModel(Qwen3PreTrainedModel):

    config_class = LigerQwen3GLAConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["Qwen3DecoderLayer"]
    _skip_keys_device_placement = "past_key_values"

class LigerQwen3GLAModel(Qwen3Model, LigerQwen3PreTrainedModel):

    def __init__(self, config: LigerQwen3GLAConfig):
        super().__init__(config)

        self.layers = nn.ModuleList(
            [LigerQwen3DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **flash_attn_kwargs,
        ...
        # swg: line 335: attention_mask=attention_mask
...
class LigerQwen3GLAForCausalLM(LigerQwen3PreTrainedModel, Qwen3ForCausalLM, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    _tp_plan = {"lm_head": "colwise_rep"}

    def __init__(self, config):
        super().__init__(config)
        self.model = LigerQwen3GLAModel(config)
