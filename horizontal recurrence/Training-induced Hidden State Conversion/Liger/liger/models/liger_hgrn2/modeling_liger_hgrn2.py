...
from liger.models.liger_hgrn2 import LigerHGRN2Config

logger = logging.get_logger(__name__)


class LigerHGRN2Attention(nn.Module):
    def __init__(
        self, 
        config: LigerHGRN2Config,
    ...
    def forward(
        ...
        **kwargs,
        ...
        f = rearrange(f, 'b n (h m) -> b h n m', h=self.num_key_value_heads)

        k = repeat_kv(k, self.num_key_value_groups)
        v = repeat_kv(v, self.num_key_value_groups)
        f = repeat_kv(f, self.num_key_value_groups)
...
class LigerHGRN2DecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: LigerHGRN2Config, layer_idx: int):
        super().__init__(config, layer_idx)
        self.hidden_size = config.hidden_size
        self.self_attn = LigerHGRN2Attention(config=config, layer_idx=layer_idx)
        self.mlp = LlamaMLP(config)
        self.input_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)


class LigerHGRN2PreTrainedModel(LlamaPreTrainedModel):

    config_class = LigerHGRN2Config
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ['LigerHGRN2DecoderLayer']
    _skip_keys_device_placement = "past_key_values"

class LigerHGRN2Model(LlamaModel, LigerHGRN2PreTrainedModel):

    def __init__(self, config: LigerHGRN2Config):
        ...
        self.layers = nn.ModuleList(
            [LigerHGRN2DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class LigerHGRN2ForCausalLM(LlamaForCausalLM, LigerHGRN2PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LigerHGRN2Model(config)
        self.vocab_size = config.vocab_size
