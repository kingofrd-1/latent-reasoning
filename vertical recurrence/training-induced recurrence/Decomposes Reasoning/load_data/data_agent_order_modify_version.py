...
def validate_cot_steps(cot_steps):
    """
    Validate the quality of generated CoT steps.
    
    Args:
        cot_steps: List of reasoning steps
        
    Returns:
        bool: True if valid, False if needs regeneration
    """
    if not cot_steps or len(cot_steps) == 0:
        print("Validation failed: No cot_steps generated")
        return False
    ...
    # Check each step and collect indices
    for i, step in enumerate(cot_steps):
        if '[rag]:' in step:
            # Extract content after [rag]:
            content = step.split('[rag]:', 1)[1].strip()
            if not content:  # Empty content
                print(f"Validation failed: Empty [rag] content at step {i}: {step}")
                return False
            rag_steps.append(content)
            rag_indices.append(i)
        elif '[reason]:' in step:
            # Extract content after [reason]:
            content = step.split('[reason]:', 1)[1].strip()
            if not content:  # Empty content
                print(f"Validation failed: Empty [reason] content at step {i}: {step}")
                return False
            reason_steps.append(content)
            reason_indices.append(i)
        else:
            print(f"Validation failed: Step without proper tag at {i}: {step}")
            return False
    
    # Must have at least one [rag] and one [reason]
    if len(rag_steps) == 0:
        print("Validation failed: No [rag] steps found")
        return False
    
    if len(reason_steps) == 0:
        print("Validation failed: No [reason] steps found")
        return False
    
    # At least one [rag] must come before the first [reason]
    first_reason_index = min(reason_indices)
    rag_before_reason = any(rag_idx < first_reason_index for rag_idx in rag_indices)
    
    if not rag_before_reason:
        print("Validation failed: No [rag] steps before first [reason] step")
        return False
    
    print(f"Validation passed: {len(rag_steps)} [rag] steps, {len(reason_steps)} [reason] steps")
    return True
...
def get_label(input_string, answer):
    ...
    cot_steps = extract_labeled_content_as_list(planing)
    
    # Validate the generated steps
    if not validate_cot_steps(cot_steps):
        print("Generated steps failed validation, returning empty list for retry")
        return []
...
def generate_StrategyQA_agent(type):
    ...
    dict = []
    json_file = "load_data/dataset_folder/StrategyQA_{}.json".format(type)
        ...
        while retry_count < max_retries:
            try:
                ...
                if(len(cot_steps) > 0):
                    ...
                    retry_count +=1
                    print(f"Validation failed for question: {question}. Attempt {retry_count} of {max_retries}. Retrying...")
            except Exception as e:
                retry_count += 1
                print(f"Error occurred while processing question: {question}. Attempt {retry_count} of {max_retries}. Error: {e}")
        
        # if the maximum number of retries is reached, save with whatever we have
        if retry_count == max_retries:
                print(f"Warning: Maximum retries ({max_retries}) reached for question: {question}")
                print(f"Saving data with potentially low quality cot_steps")
                # Use empty list if cot_steps is None
...
def generate_MMLU_pro_agent(split):
    ...
    dict = []
    json_file = "load_data/dataset_folder/MMLU_Pro_{}.json".format(type)
...
def generate_CommensenQA_agent(split):
    ...
    if type =="train":
        ...
        dict = []
        json_file = "load_data/dataset_folder/commonsense_qa_{}.json".format(type)
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if (len(cot_steps) > 0):
                        ...
                        retry_count += 1
                        print(
                            f"Validation failed for question: {question}. Attempt {retry_count} of {max_retries}. Retrying...")
                except Exception as e:
                    retry_count += 1
                    print(
                        f"Error occurred while processing question: {question}. Attempt {retry_count} of {max_retries}. Error: {e}")
        ...
        data = load_dataset("tau/commonsense_qa")[type]
        print(data)
        json_file = "load_data/dataset_folder/commonsense_qa_test_clean_CC.json"
        dict = []
        for example in tqdm(data):
            q = example['question']
            choices = example['choices']['text']
...
def generate_truthfulqa_agent(split):
    ...
    if type =="train":
        ...
        json_file = "load_data/dataset_folder/truthful_qa_{}.json".format(type)
        data = ds['validation']
        ...
        ds = load_dataset("truthfulqa/truthful_qa", "multiple_choice")
        json_file = "load_data/dataset_folder/truthful_qa_{}_clean_CC.json".format(type)
        data = ds['validation']
...
def main(args):
    ...
    file = os.path.join("load_data", "dataset_folder", "{}_{}.json".format(args.dataset, args.mode))
    clean_file = os.path.join("load_data", "dataset_folder", "{}_{}.json".format(args.dataset, args.mode + "_clean_CC"))
